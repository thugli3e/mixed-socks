...
func (s *SocksServer) listenTcpServer(ctx context.Context) error {
	conn, err := mux.Listen(ctx, "tcp", s.sockIp+":"+strconv.Itoa(s.port))
	if err != nil {
		logrus.Infoln("connect error", err)
		return err
	}
	logrus.Infoln("listen tcp:" + conn.Addr().String())
	for {
		c, err := conn.Accept()
		if err != nil {
